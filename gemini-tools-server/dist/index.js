    ...
    try {
        switch (name) {
            case 'dependency_hologram': {
                const target = argsTyped.target;
                const depth = argsTyped.depth || 5;
                const includeHidden = argsTyped.include_hidden ?? true;
                const outputFormat = argsTyped.output_format || 'text';
            ...
            case 'failure_to_test_transmuter': {
                const crashData = argsTyped.crash_data;
                const framework = argsTyped.test_framework || 'jest';
                if (!crashData) {
                    return {
                        content: [{
                                type: 'text',
                ...
                if (framework === 'jest' || framework === 'vitest') {
                    ...
                    if (envEntries.length > 0) {
                        testCode += envEntries.map(([k, v]) => '    process.env.' + k + ' = ' + JSON.stringify(v) + ';').join('\n');
                        testCode += '\n';
                    }
                    ...
                    const envEntries = Object.entries(crashData.environment || {});
                    if (envEntries.length > 0) {
                        testCode += envEntries.map(([k, v]) => "    os.environ[\"" + k + "\"] = " + JSON.stringify(v)).join('\n');
                        testCode += '\n';
                    }
            ...
            case 'trace_replay_debugger': {
                const traceData = argsTyped.trace_data;
                const testName = argsTyped.test_name || 'replay_test';
